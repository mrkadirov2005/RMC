...
const OwnerManager = memo(() => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      if (activeTab !== 'centers' && !activeCenterId) {
        showToast.error('Center scope required. Please select an active center.');
        setData([]);
        setLoading(false);
        return;
      }
    ...
    const loadCenters = async () => {
      try {
        ...
        if (!activeCenterId && centers.length > 0) {
          ...
          if (firstId) {
            setActiveCenterId(Number(firstId));
          }
        }
      } catch {
        setCenterOptions([]);
  ...
  const getRowId = (item: any) => item.id || item.superuser_id || item.teacher_id || item.student_id || item.center_id;
  ...
  const handleResetPassword = async (item: any) => {
    ...
    const username = (
      suggestedUsername ||
      window.prompt('Enter username for password reset', '') || ''
    ).trim();
    if (!username) {
    ...
    try {
      if (activeTab === 'teachers') {
        await teacherAPI.setPassword(Number(id), { username, password: tempPassword });
      } else {
        await studentAPI.setPassword(Number(id), { username, password: tempPassword });
      }
  ...
  return (
    <div className="owner-manager">
      ...
      {activeTab !== 'centers' && (
        <div className="mb-4">
          <label htmlFor="active_center_id" className="block text-sm font-medium mb-1">
            Active Center
          </label>
          <select
            id="active_center_id"
          ...
          >
            ...
            {centerOptions.map((center) => (
              <option key={center.center_id || center.id} value={center.center_id || center.id}>
                {center.center_name || center.name || `Center ${center.center_id || center.id}`}
              </option>
            ))}
          </select>
        </div>
