...
export const DiscountStatsPanel = ({ collections }: Props) => {
  const stats = useMemo(() => {
    ...
    const serial = discounted.filter((payment) => payment?.discount_kind === 'serial_discount');
    const monthly = discounted.filter((payment) => payment?.discount_kind === 'monthly_discount');
    const totalDiscount = discounted.reduce((sum, payment) => sum + Number(payment?.discount_amount || 0), 0);
    const originalTotal = discounted.reduce((sum, payment) => sum + Number(payment?.original_amount || payment?.amount || 0), 0);
    const finalTotal = discounted.reduce((sum, payment) => sum + Number(payment?.final_amount || payment?.amount || 0), 0);
    const complete = discounted.filter((payment) => payment?.is_complete === true);
    const partial = discounted.filter((payment) => payment?.is_complete === false && Number(payment?.amount || 0) > 0);
    const unpaid = discounted.filter((payment) => Number(payment?.amount || 0) <= 0);
  ...
  const kindRows = [
    ...
    { label: 'Monthly', value: stats.monthly.length, color: colors.monthly },
  ].filter((row) => row.value > 0);

  return (
    <div className="space-y-4">
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <MetricCard icon={<BadgePercent className="h-5 w-5" />} label="Discounted payments" value={stats.discounted.length.toLocaleString()} tone="from-blue-600 to-cyan-600" />
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <ChartCard title="Discount types" rows={kindRows} centerValue={stats.discounted.length.toLocaleString()} />
        <ChartCard title="Payment status" rows={statusRows} centerValue={stats.discounted.length.toLocaleString()} />
      </div>
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-white/[0.04]">
        <div className="mb-3 flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs font-semibold text-slate-500">Top payments by discount amount.</p>
          </div>
        </div>
        {stats.top.length === 0 ? (
          <div className="rounded-md border border-dashed border-slate-300 py-8 text-center text-sm font-semibold text-slate-500">
            No discounted payments yet.
          </div>
        ) : (
          <div className="space-y-2">
            {stats.top.map((payment, index) => (
              <div key={payment.payment_id || index} className="flex flex-wrap items-center gap-3 rounded-md border border-slate-100 bg-slate-50 px-3 py-2 text-sm dark:border-white/10 dark:bg-white/[0.03]">
                ...
                <span className="min-w-44 font-black text-slate-900 dark:text-white">
                  {[payment.student_first_name, payment.student_last_name].filter(Boolean).join(' ') || `Student #${payment.student_id}`}
                </span>
                <span className="rounded bg-blue-100 px-2 py-1 text-xs font-black text-blue-700">{payment.discount_kind || 'discount'}</span>
                <span className="rounded bg-rose-100 px-2 py-1 text-xs font-black text-rose-700">-{formatMoney(Number(payment.discount_amount || 0))}</span>
                <span className="rounded bg-emerald-100 px-2 py-1 text-xs font-black text-emerald-700">final {formatMoney(Number(payment.final_amount || payment.amount || 0))}</span>
              </div>
            ))}
          </div>
        )}
