...
const formatMoney = (value: unknown) => {
  ...
  return `UZS ${amount.toLocaleString()}`;
};
...
const getPaymentStatus = (payment: Partial<Payment>) =>
  String(payment.status || payment.payment_status || 'Pending').trim() || 'Pending';
const getStatusClasses = (status: string) => {
  const normalized = status.toLowerCase();
  if (normalized === 'completed' || normalized === 'paid') return 'bg-emerald-100 text-emerald-800';
  if (normalized === 'failed' || normalized === 'cancelled') return 'bg-rose-100 text-rose-800';
                    ...
                    <div className="overflow-hidden rounded-lg border">
                      <Table className="text-xs">
                        <TableHeader className="bg-slate-50/90 dark:bg-muted/30">
                          <TableRow>
                            <TableHead className="h-8 px-2">Month</TableHead>
                        ...
                        <TableBody>
                          {paymentHistoryLoading ? (
                            ...
                            historyMonths.map((month) => {
                              ...
                              if (rows.length === 0) {
                                return (
                                  <TableRow key={getMonthKey(month)}>
                                    <TableCell className="px-2 py-2 font-semibold">{getMonthLabel(month)}</TableCell>
                                    ...
                                    <TableCell className="px-2 py-2">
                                      <span className="rounded-md bg-slate-100 px-2 py-1 font-semibold text-slate-700">No payment</span>
                                    </TableCell>
                                  </TableRow>
                                );
                              }
                              ...
                              return rows.map((payment, index) => {
                                ...
                                return (
                                  <TableRow key={`${getMonthKey(month)}-${payment.payment_id || payment.id || index}`}>
                                    <TableCell className="px-2 py-2 font-semibold">{index === 0 ? getMonthLabel(month) : ''}</TableCell>
                                    <TableCell className="px-2 py-2">{formatDate(payment.payment_date)}</TableCell>
                                    <TableCell className="max-w-[140px] truncate px-2 py-2 font-mono text-[11px]">{payment.receipt_number || '-'}</TableCell>
                                    <TableCell className="px-2 py-2 font-semibold">{formatMoney(payment.amount)}</TableCell>
                                    <TableCell className="px-2 py-2">{payment.payment_method || '-'}</TableCell>
                                    <TableCell className="px-2 py-2">{payment.payment_type || '-'}</TableCell>
                                    <TableCell className="px-2 py-2">
                                      <span className={`rounded-md px-2 py-1 font-semibold ${getStatusClasses(status)}`}>{status}</span>
                                    </TableCell>
                                  </TableRow>
                                );
                              });
