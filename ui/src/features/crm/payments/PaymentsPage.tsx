...
const PaymentsPage = () => {
  ...
  }, [user?.userType, paymentAccess.isAuthenticated]);
  ...
  const handleOpenModal = (payment?: Payment) => {
    ...
    const defaultCenterId = user?.center_id;
    if (payment) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...formData,
      center_id: formData.center_id ?? user?.center_id,
    };
    if (editingId) {
  ...
  const getTotalAmountForClass = (classId: number): number => {
    const studentIds = getStudentIdsForClass(classId);
    return state.items
      .filter((p) => studentIds.includes(p.student_id) && (p.status || p.payment_status) === 'Completed')
  ...
  const getTotalAmountForStudent = (studentId: number): number => {
    return state.items
      .filter((p) => p.student_id === studentId && (p.status || p.payment_status) === 'Completed')
      .reduce((sum, p) => sum + (p.amount || 0), 0);
  };
