...
export const CalendarEventButton = ({ event, compact = false, onSelect }: { event: CalendarEvent; compact?: boolean; onSelect: (event: CalendarEvent) => void }) => (
  <button type="button" onClick={() => onSelect(event)} data-testid={`calendar-event-${event.event_id}`} aria-label={`${event.class_name}, ${event.start_time?.slice(0, 5)} to ${event.end_time?.slice(0, 5)}, ${String(event.status || 'planned').replace('_', ' ')}`} className={cn('w-full rounded-md border border-transparent px-2 py-1.5 text-left transition hover:border-current/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring', statusTone(event.status || 'planned'))}>
