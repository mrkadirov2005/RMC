  ...
  return (
    <Card className="overflow-hidden">
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          <CardTitle className="text-base">{t('Student Payments')}</CardTitle>
          <p className="text-xs text-muted-foreground">{t("This month's expected vs collected tuition")}</p>
        </div>
      ...
      <CardContent className="space-y-5">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <div className="rounded-lg border bg-muted/30 p-3">
            <p className="text-xs text-muted-foreground">{t('Should pay')}</p>
            <p className="mt-1 text-xl font-bold">{formatMoney(stats.expectedPaymentsThisMonth)}</p>
          </div>
          <div className="rounded-lg border bg-emerald-500/5 p-3">
            <p className="text-xs text-emerald-700 dark:text-emerald-300">{t('Paid')}</p>
          ...
          <div className="rounded-lg border bg-rose-500/5 p-3">
            <p className="text-xs text-rose-700 dark:text-rose-300">{t('Still unpaid')}</p>
        ...
        <div>
          <div className="mb-2 flex items-center justify-between text-xs text-muted-foreground">
            <span>{stats.paymentCollectionRate}% {t('collected')}</span>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div className="flex items-center gap-3 rounded-lg border p-3">
            ...
            <div>
              <p className="text-sm font-semibold">{stats.paidStudentsThisMonth} {t('students paid')}</p>
              <p className="text-xs text-muted-foreground">{t('Covered their expected monthly payment')}</p>
            </div>
          </div>
          <div className="flex items-center gap-3 rounded-lg border p-3">
            ...
            <div>
              <p className="text-sm font-semibold">{stats.unpaidStudentsThisMonth} {t('students remaining')}</p>
              <p className="text-xs text-muted-foreground">{t('Still below their expected monthly payment')}</p>
            </div>
          </div>
        </div>
      </CardContent>
