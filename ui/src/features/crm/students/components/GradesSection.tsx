...
export const GradesSection = ({ grades, onRefresh, studentId, classId, teacherId, centerId }: GradesSectionProps) => {
  ...
  const getSubjectName = (subjectId?: number | string) => {
    ...
    if (typeof subjectId === 'string') {
      ...
      if (Number.isNaN(asNumber)) return subjectId;
      const subject = subjects.find((s) => Number(s.subject_id || s.id) === asNumber);
      return subject?.subject_name || subjectId;
    }
    const subject = subjects.find((s) => Number(s.subject_id || s.id) === Number(subjectId));
              ...
              >
                ...
                <SelectContent>
                  {subjects.map((subject) => (
                    <SelectItem key={subject.subject_id || subject.id} value={subject.subject_name}>
