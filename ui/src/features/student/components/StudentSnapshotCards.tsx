...
export const StudentSnapshotCards = ({ student, teacher, classInfo, subjects, t }: StudentSnapshotCardsProps) => (
  <div className="grid grid-cols-1 gap-4 animate-fade-in animation-delay-400 md:grid-cols-2 lg:grid-cols-3">
    ...
    <Card>
      <CardHeader>
        <CardTitle className="text-base text-foreground">{t('Subjects')}</CardTitle>
      </CardHeader>
      <CardContent className="space-y-2 text-sm">
        {subjects.length === 0 ? (
          ...
          subjects.slice(0, 6).map((subject) => (
            <div key={subject.subject_id || subject.id} className="flex items-center justify-between rounded-md bg-muted px-3 py-2">
              <span className="text-foreground">{subject.subject_name}</span>
  ...
  <Card>
    ...
    <CardContent className="space-y-3 text-sm">
      {rows.filter(([, value]) => value).map(([Icon, value], index) => (
        <div key={index} className="flex items-center gap-2 rounded-md bg-muted px-3 py-2">
