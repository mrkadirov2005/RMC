...
export const MetricCard = ({
  ...
  return (
    <Comp
      ...
      onClick={onClick}
      className={cn(
        'relative h-full min-h-[114px] overflow-hidden rounded-xl border px-4 py-3.5 text-left text-white shadow-md transition-all duration-200',
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <p className={cn('min-h-[1.75rem] line-clamp-2 text-[11px] font-semibold uppercase leading-4', toneTextClasses[tone])}>
            {label}
          </p>
          <div className="mt-1 text-2xl font-bold tracking-normal text-white">{value}</div>
          {detail ? (
            <div className={cn('mt-1 min-h-[1.5rem] line-clamp-2 text-[11px] leading-4', toneTextClasses[tone])}>
              {detail}
            </div>
          ) : (
        ...
        {Icon && (
          <div className={cn('flex h-8 w-8 shrink-0 items-center justify-center rounded-md ring-1', toneClasses[tone])}>
