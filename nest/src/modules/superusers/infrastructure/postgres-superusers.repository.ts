...
import { superusers } from '../../../database/schema';
import type { SuperusersRecord } from '../domain/superusers.entity';
import type { SuperusersRepositoryPort } from '../domain/superusers.repository.port';

const table: any = superusers;
const pk: any = table.superuserId;
const centerColumn: any = table.centerId;
const updatedAtColumn: any = table.updatedAt;

@Injectable()
export class PostgresSuperusersRepository implements SuperusersRepositoryPort {
  constructor(@Inject(DRIZZLE_DB) private readonly db: any) {}

  async findAll(centerId?: number): Promise<SuperusersRecord[]> {
  ...
  async findById(id: number, centerId?: number): Promise<SuperusersRecord | null> {
  ...
  async create(payload: Record<string, unknown>, centerId?: number): Promise<SuperusersRecord> {
  ...
  async update(id: number, payload: Record<string, unknown>, centerId?: number): Promise<SuperusersRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
  ...
  async delete(id: number, centerId?: number): Promise<SuperusersRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
