...
import type { SearchRecord } from '../domain/search.entity';
import type { SearchRepositoryPort } from '../domain/search.repository.port';
...
export class PostgresSearchRepository implements SearchRepositoryPort {
  ...
  async findAll(centerId?: number): Promise<SearchRecord[]> {
  ...
  async findById(id: number, centerId?: number): Promise<SearchRecord | null> {
  ...
  async create(payload: Record<string, unknown>, centerId?: number): Promise<SearchRecord> {
  ...
  async update(id: number, payload: Record<string, unknown>, centerId?: number): Promise<SearchRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
  ...
  async delete(id: number, centerId?: number): Promise<SearchRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
