...
import { SEARCH_REPOSITORY, type SearchRepositoryPort } from '../domain/search.repository.port';

@Injectable()
export class SearchService {
  constructor(@Inject(SEARCH_REPOSITORY) private readonly repository: SearchRepositoryPort) {}
  ...
  async get(id: number, centerId?: number) {
    ...
    if (!row) throw new NotFoundException('Search record not found');
    return row;
  }
