...
import type { InvoicesRecord } from '../domain/invoices.entity';
import type { InvoicesRepositoryPort } from '../domain/invoices.repository.port';

const table: any = invoices;
const pk: any = table.invoiceId;
const centerColumn: any = table.centerId;
const updatedAtColumn: any = table.updatedAt;

@Injectable()
export class PostgresInvoicesRepository implements InvoicesRepositoryPort {
  constructor(@Inject(DRIZZLE_DB) private readonly db: any) {}

  async findAll(centerId?: number): Promise<InvoicesRecord[]> {
  ...
  async findById(id: number, centerId?: number): Promise<InvoicesRecord | null> {
  ...
  async create(payload: Record<string, unknown>, centerId?: number): Promise<InvoicesRecord> {
  ...
  async update(id: number, payload: Record<string, unknown>, centerId?: number): Promise<InvoicesRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
  ...
  async delete(id: number, centerId?: number): Promise<InvoicesRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
