...
import { INVOICES_REPOSITORY, type InvoicesRepositoryPort } from '../domain/invoices.repository.port';

@Injectable()
export class InvoicesService {
  constructor(@Inject(INVOICES_REPOSITORY) private readonly repository: InvoicesRepositoryPort) {}
  ...
  async get(id: number, centerId?: number) {
    ...
    if (!row) throw new NotFoundException('Invoices record not found');
    return row;
  }
