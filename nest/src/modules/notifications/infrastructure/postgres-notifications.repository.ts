...
import type { NotificationsRecord } from '../domain/notifications.entity';
import type { NotificationsRepositoryPort } from '../domain/notifications.repository.port';

const table: any = notifications;
const pk: any = table.notificationId;
const centerColumn: any = table.centerId;
const updatedAtColumn: any = table.updatedAt;

@Injectable()
export class PostgresNotificationsRepository implements NotificationsRepositoryPort {
  constructor(@Inject(DRIZZLE_DB) private readonly db: any) {}

  async findAll(centerId?: number): Promise<NotificationsRecord[]> {
  ...
  async findById(id: number, centerId?: number): Promise<NotificationsRecord | null> {
  ...
  async create(payload: Record<string, unknown>, centerId?: number): Promise<NotificationsRecord> {
  ...
  async update(id: number, payload: Record<string, unknown>, centerId?: number): Promise<NotificationsRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
  ...
  async delete(id: number, centerId?: number): Promise<NotificationsRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
