...
import type { RoomsRecord } from '../domain/rooms.entity';
import type { RoomsRepositoryPort } from '../domain/rooms.repository.port';

const table: any = rooms;
const pk: any = table.roomId;
const centerColumn: any = table.centerId;
const updatedAtColumn: any = table.updatedAt;

@Injectable()
export class PostgresRoomsRepository implements RoomsRepositoryPort {
  constructor(@Inject(DRIZZLE_DB) private readonly db: any) {}

  async findAll(centerId?: number): Promise<RoomsRecord[]> {
  ...
  async findById(id: number, centerId?: number): Promise<RoomsRecord | null> {
  ...
  async create(payload: Record<string, unknown>, centerId?: number): Promise<RoomsRecord> {
  ...
  async update(id: number, payload: Record<string, unknown>, centerId?: number): Promise<RoomsRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
  ...
  async delete(id: number, centerId?: number): Promise<RoomsRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
