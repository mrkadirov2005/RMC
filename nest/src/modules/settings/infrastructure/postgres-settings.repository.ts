...
import type { SettingsRecord } from '../domain/settings.entity';
import type { SettingsRepositoryPort } from '../domain/settings.repository.port';

const table: any = appSettings;
const pk: any = table.settingId;
...
export class PostgresSettingsRepository implements SettingsRepositoryPort {
  ...
  async findAll(centerId?: number): Promise<SettingsRecord[]> {
  ...
  async findById(id: number, centerId?: number): Promise<SettingsRecord | null> {
  ...
  async create(payload: Record<string, unknown>, centerId?: number): Promise<SettingsRecord> {
  ...
  async update(id: number, payload: Record<string, unknown>, centerId?: number): Promise<SettingsRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
  ...
  async delete(id: number, centerId?: number): Promise<SettingsRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
