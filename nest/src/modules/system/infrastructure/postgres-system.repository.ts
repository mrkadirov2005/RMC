...
import type { SystemRecord } from '../domain/system.entity';
import type { SystemRepositoryPort } from '../domain/system.repository.port';

const table: any = requestLogs;
const pk: any = table.logId;
const centerColumn: any = table.centerId;
const updatedAtColumn: any = table.updatedAt;

@Injectable()
export class PostgresSystemRepository implements SystemRepositoryPort {
  constructor(@Inject(DRIZZLE_DB) private readonly db: any) {}

  async findAll(centerId?: number): Promise<SystemRecord[]> {
  ...
  async findById(id: number, centerId?: number): Promise<SystemRecord | null> {
  ...
  async create(payload: Record<string, unknown>, centerId?: number): Promise<SystemRecord> {
  ...
  async update(id: number, payload: Record<string, unknown>, centerId?: number): Promise<SystemRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
  ...
  async delete(id: number, centerId?: number): Promise<SystemRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
