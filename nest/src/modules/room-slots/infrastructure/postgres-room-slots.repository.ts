...
import { roomSlots } from '../../../database/schema';
import type { RoomSlotsRecord } from '../domain/room-slots.entity';
import type { RoomSlotsRepositoryPort } from '../domain/room-slots.repository.port';

const table: any = roomSlots;
const pk: any = table.slotId;
const centerColumn: any = table.centerId;
const updatedAtColumn: any = table.updatedAt;

@Injectable()
export class PostgresRoomSlotsRepository implements RoomSlotsRepositoryPort {
  constructor(@Inject(DRIZZLE_DB) private readonly db: any) {}

  async findAll(centerId?: number): Promise<RoomSlotsRecord[]> {
  ...
  async findById(id: number, centerId?: number): Promise<RoomSlotsRecord | null> {
  ...
  async create(payload: Record<string, unknown>, centerId?: number): Promise<RoomSlotsRecord> {
  ...
  async update(id: number, payload: Record<string, unknown>, centerId?: number): Promise<RoomSlotsRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
  ...
  async delete(id: number, centerId?: number): Promise<RoomSlotsRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
