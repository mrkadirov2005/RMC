...
import type { ParentsRecord } from '../domain/parents.entity';
import type { ParentsRepositoryPort } from '../domain/parents.repository.port';

const table: any = parents;
const pk: any = table.parentId;
const centerColumn: any = table.centerId;
const updatedAtColumn: any = table.updatedAt;

@Injectable()
export class PostgresParentsRepository implements ParentsRepositoryPort {
  constructor(@Inject(DRIZZLE_DB) private readonly db: any) {}

  async findAll(centerId?: number): Promise<ParentsRecord[]> {
  ...
  async findById(id: number, centerId?: number): Promise<ParentsRecord | null> {
  ...
  async create(payload: Record<string, unknown>, centerId?: number): Promise<ParentsRecord> {
  ...
  async update(id: number, payload: Record<string, unknown>, centerId?: number): Promise<ParentsRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
  ...
  async delete(id: number, centerId?: number): Promise<ParentsRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
