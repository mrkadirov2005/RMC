...
import type { ImportExportRecord } from '../domain/import-export.entity';
import type { ImportExportRepositoryPort } from '../domain/import-export.repository.port';

const table: any = importJobs;
const pk: any = table.importJobId;
const centerColumn: any = table.centerId;
const updatedAtColumn: any = table.updatedAt;

@Injectable()
export class PostgresImportExportRepository implements ImportExportRepositoryPort {
  constructor(@Inject(DRIZZLE_DB) private readonly db: any) {}

  async findAll(centerId?: number): Promise<ImportExportRecord[]> {
  ...
  async findById(id: number, centerId?: number): Promise<ImportExportRecord | null> {
  ...
  async create(payload: Record<string, unknown>, centerId?: number): Promise<ImportExportRecord> {
  ...
  async update(id: number, payload: Record<string, unknown>, centerId?: number): Promise<ImportExportRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
  ...
  async delete(id: number, centerId?: number): Promise<ImportExportRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
