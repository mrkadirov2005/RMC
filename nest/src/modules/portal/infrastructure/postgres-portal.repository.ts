...
import type { PortalRecord } from '../domain/portal.entity';
import type { PortalRepositoryPort } from '../domain/portal.repository.port';
...
@Injectable()
export class PostgresPortalRepository implements PortalRepositoryPort {
  constructor(@Inject(DRIZZLE_DB) private readonly db: any) {}

  async findAll(centerId?: number): Promise<PortalRecord[]> {
  ...
  async findById(id: number, centerId?: number): Promise<PortalRecord | null> {
  ...
  async create(payload: Record<string, unknown>, centerId?: number): Promise<PortalRecord> {
  ...
  async update(id: number, payload: Record<string, unknown>, centerId?: number): Promise<PortalRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
  ...
  async delete(id: number, centerId?: number): Promise<PortalRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
