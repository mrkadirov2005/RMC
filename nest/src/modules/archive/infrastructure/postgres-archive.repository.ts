...
import type { ArchiveRecord } from '../domain/archive.entity';
import type { ArchiveRepositoryPort } from '../domain/archive.repository.port';

const table: any = students;
const pk: any = table.studentId;
const centerColumn: any = table.centerId;
const updatedAtColumn: any = table.updatedAt;

@Injectable()
export class PostgresArchiveRepository implements ArchiveRepositoryPort {
  constructor(@Inject(DRIZZLE_DB) private readonly db: any) {}

  async findAll(centerId?: number): Promise<ArchiveRecord[]> {
  ...
  async findById(id: number, centerId?: number): Promise<ArchiveRecord | null> {
  ...
  async create(payload: Record<string, unknown>, centerId?: number): Promise<ArchiveRecord> {
  ...
  async update(id: number, payload: Record<string, unknown>, centerId?: number): Promise<ArchiveRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
  ...
  async delete(id: number, centerId?: number): Promise<ArchiveRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
