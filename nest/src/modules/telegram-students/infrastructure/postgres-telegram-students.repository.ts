...
import { telegramStudents } from '../../../database/schema';
import type { TelegramStudentsRecord } from '../domain/telegram-students.entity';
import type { TelegramStudentsRepositoryPort } from '../domain/telegram-students.repository.port';

const table: any = telegramStudents;
const pk: any = table.telegramStudentId;
const centerColumn: any = table.centerId;
const updatedAtColumn: any = table.updatedAt;

@Injectable()
export class PostgresTelegramStudentsRepository implements TelegramStudentsRepositoryPort {
  constructor(@Inject(DRIZZLE_DB) private readonly db: any) {}

  async findAll(centerId?: number): Promise<TelegramStudentsRecord[]> {
  ...
  async findById(id: number, centerId?: number): Promise<TelegramStudentsRecord | null> {
  ...
  async create(payload: Record<string, unknown>, centerId?: number): Promise<TelegramStudentsRecord> {
  ...
  async update(id: number, payload: Record<string, unknown>, centerId?: number): Promise<TelegramStudentsRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
  ...
  async delete(id: number, centerId?: number): Promise<TelegramStudentsRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
