...
import type { AttendanceRecord } from '../domain/attendance.entity';
import type { AttendanceRepositoryPort } from '../domain/attendance.repository.port';

const table: any = attendance;
const pk: any = table.attendanceId;
...
export class PostgresAttendanceRepository implements AttendanceRepositoryPort {
  ...
  async findAll(centerId?: number): Promise<AttendanceRecord[]> {
  ...
  async findById(id: number, centerId?: number): Promise<AttendanceRecord | null> {
  ...
  async create(payload: Record<string, unknown>, centerId?: number): Promise<AttendanceRecord> {
  ...
  async update(id: number, payload: Record<string, unknown>, centerId?: number): Promise<AttendanceRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
  ...
  async delete(id: number, centerId?: number): Promise<AttendanceRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
