...
import type { ClassesRecord } from '../domain/classes.entity';
import type { ClassesRepositoryPort } from '../domain/classes.repository.port';

const table: any = classes;
const pk: any = table.classId;
const centerColumn: any = table.centerId;
const updatedAtColumn: any = table.updatedAt;

@Injectable()
export class PostgresClassesRepository implements ClassesRepositoryPort {
  constructor(@Inject(DRIZZLE_DB) private readonly db: any) {}

  async findAll(centerId?: number): Promise<ClassesRecord[]> {
  ...
  async findById(id: number, centerId?: number): Promise<ClassesRecord | null> {
  ...
  async create(payload: Record<string, unknown>, centerId?: number): Promise<ClassesRecord> {
  ...
  async update(id: number, payload: Record<string, unknown>, centerId?: number): Promise<ClassesRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
  ...
  async delete(id: number, centerId?: number): Promise<ClassesRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
