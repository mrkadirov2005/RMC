...
import type { ReportsRecord } from '../domain/reports.entity';
import type { ReportsRepositoryPort } from '../domain/reports.repository.port';

const table: any = payments;
const pk: any = table.paymentId;
...
export class PostgresReportsRepository implements ReportsRepositoryPort {
  ...
  async findAll(centerId?: number): Promise<ReportsRecord[]> {
  ...
  async findById(id: number, centerId?: number): Promise<ReportsRecord | null> {
  ...
  async create(payload: Record<string, unknown>, centerId?: number): Promise<ReportsRecord> {
  ...
  async update(id: number, payload: Record<string, unknown>, centerId?: number): Promise<ReportsRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
  ...
  async delete(id: number, centerId?: number): Promise<ReportsRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
