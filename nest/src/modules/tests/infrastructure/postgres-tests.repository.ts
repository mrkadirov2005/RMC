...
import type { TestsRecord } from '../domain/tests.entity';
import type { TestsRepositoryPort } from '../domain/tests.repository.port';

const table: any = tests;
const pk: any = table.testId;
const centerColumn: any = table.centerId;
const updatedAtColumn: any = table.updatedAt;

@Injectable()
export class PostgresTestsRepository implements TestsRepositoryPort {
  constructor(@Inject(DRIZZLE_DB) private readonly db: any) {}

  async findAll(centerId?: number): Promise<TestsRecord[]> {
  ...
  async findById(id: number, centerId?: number): Promise<TestsRecord | null> {
  ...
  async create(payload: Record<string, unknown>, centerId?: number): Promise<TestsRecord> {
  ...
  async update(id: number, payload: Record<string, unknown>, centerId?: number): Promise<TestsRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
  ...
  async delete(id: number, centerId?: number): Promise<TestsRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
