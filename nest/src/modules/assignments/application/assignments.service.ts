...
import { ASSIGNMENTS_REPOSITORY, type AssignmentsRepositoryPort } from '../domain/assignments.repository.port';

@Injectable()
export class AssignmentsService {
  constructor(@Inject(ASSIGNMENTS_REPOSITORY) private readonly repository: AssignmentsRepositoryPort) {}
  ...
  async get(id: number, centerId?: number) {
    ...
    if (!row) throw new NotFoundException('Assignments record not found');
    return row;
  }
