...
import type { AssignmentsRecord } from '../domain/assignments.entity';
import type { AssignmentsRepositoryPort } from '../domain/assignments.repository.port';

const table: any = assignments;
const pk: any = table.assignmentId;
const centerColumn: any = table.centerId;
const updatedAtColumn: any = table.updatedAt;

@Injectable()
export class PostgresAssignmentsRepository implements AssignmentsRepositoryPort {
  constructor(@Inject(DRIZZLE_DB) private readonly db: any) {}

  async findAll(centerId?: number): Promise<AssignmentsRecord[]> {
  ...
  async findById(id: number, centerId?: number): Promise<AssignmentsRecord | null> {
  ...
  async create(payload: Record<string, unknown>, centerId?: number): Promise<AssignmentsRecord> {
  ...
  async update(id: number, payload: Record<string, unknown>, centerId?: number): Promise<AssignmentsRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
  ...
  async delete(id: number, centerId?: number): Promise<AssignmentsRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
