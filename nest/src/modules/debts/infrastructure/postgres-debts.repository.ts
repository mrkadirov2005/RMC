...
import type { DebtsRecord } from '../domain/debts.entity';
import type { DebtsRepositoryPort } from '../domain/debts.repository.port';

const table: any = debts;
const pk: any = table.debtId;
const centerColumn: any = table.centerId;
const updatedAtColumn: any = table.updatedAt;

@Injectable()
export class PostgresDebtsRepository implements DebtsRepositoryPort {
  constructor(@Inject(DRIZZLE_DB) private readonly db: any) {}

  async findAll(centerId?: number): Promise<DebtsRecord[]> {
  ...
  async findById(id: number, centerId?: number): Promise<DebtsRecord | null> {
  ...
  async create(payload: Record<string, unknown>, centerId?: number): Promise<DebtsRecord> {
  ...
  async update(id: number, payload: Record<string, unknown>, centerId?: number): Promise<DebtsRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
  ...
  async delete(id: number, centerId?: number): Promise<DebtsRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
