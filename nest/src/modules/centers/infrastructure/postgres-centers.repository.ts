...
import type { CentersRecord } from '../domain/centers.entity';
import type { CentersRepositoryPort } from '../domain/centers.repository.port';

const table: any = centers;
const pk: any = table.centerId;
const centerColumn: any = table.centerId;
const updatedAtColumn: any = table.updatedAt;

@Injectable()
export class PostgresCentersRepository implements CentersRepositoryPort {
  constructor(@Inject(DRIZZLE_DB) private readonly db: any) {}

  async findAll(centerId?: number): Promise<CentersRecord[]> {
  ...
  async findById(id: number, centerId?: number): Promise<CentersRecord | null> {
  ...
  async create(payload: Record<string, unknown>, centerId?: number): Promise<CentersRecord> {
  ...
  async update(id: number, payload: Record<string, unknown>, centerId?: number): Promise<CentersRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
  ...
  async delete(id: number, centerId?: number): Promise<CentersRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
