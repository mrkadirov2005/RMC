...
import { paymentPlans } from '../../../database/schema';
import type { PaymentPlansRecord } from '../domain/payment-plans.entity';
import type { PaymentPlansRepositoryPort } from '../domain/payment-plans.repository.port';

const table: any = paymentPlans;
const pk: any = table.planId;
const centerColumn: any = table.centerId;
const updatedAtColumn: any = table.updatedAt;

@Injectable()
export class PostgresPaymentPlansRepository implements PaymentPlansRepositoryPort {
  constructor(@Inject(DRIZZLE_DB) private readonly db: any) {}

  async findAll(centerId?: number): Promise<PaymentPlansRecord[]> {
  ...
  async findById(id: number, centerId?: number): Promise<PaymentPlansRecord | null> {
  ...
  async create(payload: Record<string, unknown>, centerId?: number): Promise<PaymentPlansRecord> {
  ...
  async update(id: number, payload: Record<string, unknown>, centerId?: number): Promise<PaymentPlansRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
  ...
  async delete(id: number, centerId?: number): Promise<PaymentPlansRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
