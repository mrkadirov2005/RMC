...
import type { AuditLogsRecord } from '../domain/audit-logs.entity';
import type { AuditLogsRepositoryPort } from '../domain/audit-logs.repository.port';

const table: any = auditLogs;
const pk: any = table.auditLogId;
...
export class PostgresAuditLogsRepository implements AuditLogsRepositoryPort {
  ...
  async findAll(centerId?: number): Promise<AuditLogsRecord[]> {
  ...
  async findById(id: number, centerId?: number): Promise<AuditLogsRecord | null> {
  ...
  async create(payload: Record<string, unknown>, centerId?: number): Promise<AuditLogsRecord> {
  ...
  async update(id: number, payload: Record<string, unknown>, centerId?: number): Promise<AuditLogsRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
  ...
  async delete(id: number, centerId?: number): Promise<AuditLogsRecord | null> {
    const filters = [eq(pk, id)];
    if (centerId && centerColumn) filters.push(eq(centerColumn, centerId));
